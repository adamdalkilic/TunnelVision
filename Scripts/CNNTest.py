...
"""
autoencoder = keras.models.load_model(r"C:\Users\dalki\PycharmProjects\Pathology\Proof Of Concept-Intracellular Parasite Detection\TrainedModels\Autoencoder.keras")
...
for i,x in enumerate(x_test):
    ...
    loss = 1000 * autoencoder.evaluate(x,x) # first arg is input, second is the label (autoencoders: output = input) I'm a broken record I know
    reconstruction = autoencoder.predict(x) # only need input for prediction, no label
    reconList.append(np.squeeze(reconstruction))
...
"""
Displaying the images that were reconstructed most accurately (should expect no malrial cells here)
"""
fig1, axs1 = plt.subplots(rows, cols)
plt.title(f"Top {numToDisplay} Lowest Error Reconstructions")
...
plt.title(f"Top {numToDisplay} Lowest Error Original Images")
axs2 = axs2.flatten()
...
plt.title(f"Top {numToDisplay} Highest Error Reconstructions")
...
plt.title(f"Top {numToDisplay} Highest Error Original Images")
axs4 = axs4.flatten()
